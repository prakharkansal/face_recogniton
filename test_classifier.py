...
def _get_test_and_train_set(input_dir, min_num_images_per_label, split_ratio=0.7):
    ...
    test_set, train_set = split_dataset(dataset, split_ratio=split_ratio)
...
def _evaluate_classifier(emb_array, label_array, classifier_filename):
    ...
    with open(classifier_filename, 'rb') as f:
        ...
        file = open('/medium-facenet-tutorial/user_name/testfile.txt','w+') 
        for i in range(len(best_class_indices)):
            file.write('%4d  %s: %.3f' % (i+1, class_names[best_class_indices[i]], best_class_probabilities[i]))
            file.write("\n")

        file.write("\n")
        file.write("\n")
        file.write("Total number of Students present : " '%s'%(i+1))
        file.close() 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-path', type=str, action='store', dest='model_path', default='/medium-facenet-tutorial/etc/20170512-110547/20170512-110547.pb',
                        help='Path to model protobuf graph')
    #parser.add_argument('--input-dir', type=str, action='store', dest='input_dir', default='/medium-facenet-tutorial/output/intermediate',
    ...
    parser.add_argument('--num-epochs', type=int, action='store', dest='num_epochs', default=5,
                        help='Path to output trained classifier model')
    parser.add_argument('--split-ratio', type=float, action='store', dest='split_ratio', default=1,
                        help='Ratio to split train/test dataset')
    parser.add_argument('--min-num-images-per-class', type=int, action='store', default=0,
                        dest='min_images_per_class', help='Minimum number of images per class')
    parser.add_argument('--classifier-path', type=str, action='store', dest='classifier_path', default='/medium-facenet-tutorial/user_name/output/classifier.pkl',
                        help='Path to output trained classifier model')
    parser.add_argument('--is-train', action='store_true', dest='is_train', default=False,
                        help='Flag to determine if train or evaluate')
    parser.add_argument('--input-dir1', type=str, action='store', default='/medium-facenet-tutorial/user_name/group_photo', dest='input_dir1')
    parser.add_argument('--output-dir1', type=str, action='store', default='/medium-facenet-tutorial/user_name/output/test', dest='output_dir1')
    parser.add_argument('--crop-dim', type=int, action='store', default=180, dest='crop_dim',
                        help='Size to crop images to')
    args = parser.parse_args()
    preprocess_test.main(args.input_dir1, args.output_dir1, args.crop_dim)

    main(input_directory=args.output_dir1, model_path=args.model_path, classifier_output_path=args.classifier_path,
    ...
    shutil.rmtree('/medium-facenet-tutorial/user_name/output/test', ignore_errors=False, onerror=None)
