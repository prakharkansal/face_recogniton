...
from medium_facenet_tutorial.align_dlib_test import AlignDlib
...
def preprocess_image(input_path, output_path, crop_dim):
    ...
    for (i, rect) in enumerate(rects):
        ...
        if image1 is not None:
            ...
            output_path1 = os.path.join(os.path.dirname(output_path) , '%s.jpg' %i )
            #print (output_path1)
            #output_path = os.path.join(output_path.)
            logger.debug('Writing processed file: {}'.format(output_path1))
